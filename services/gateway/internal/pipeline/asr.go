...
// --- faster-whisper backend (OpenAI-compatible /v1/audio/transcriptions) ---

// FasterWhisperClient sends audio to a Speaches/faster-whisper server.
...
// NewFasterWhisperClient creates a client for the faster-whisper server.
func NewFasterWhisperClient(url, model string, poolSize int) *FasterWhisperClient {
...
// Transcribe sends audio to /v1/audio/transcriptions as multipart form (file + model).
func (c *FasterWhisperClient) Transcribe(ctx context.Context, samples []float32) (*ASRResult, error) {
	start := time.Now()
	...
	req, err := http.NewRequestWithContext(ctx, "POST", c.url+"/v1/audio/transcriptions", &body)
	if err != nil {
		return nil, fmt.Errorf("create faster-whisper request: %w", err)
	}
	...
	if err != nil {
		metrics.Errors.WithLabelValues("asr", "http").Inc()
		return nil, fmt.Errorf("faster-whisper request: %w", err)
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		respBody, _ := io.ReadAll(resp.Body)
		metrics.Errors.WithLabelValues("asr", "status").Inc()
		return nil, fmt.Errorf("faster-whisper status %d: %s", resp.StatusCode, string(respBody))
	}

	var result whisperResponse
	if err = json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return nil, fmt.Errorf("decode faster-whisper response: %w", err)
	}
